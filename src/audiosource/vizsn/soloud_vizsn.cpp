...
static const char keyz[] = {P_A, P_P, P_S, P_T, P_E, P_V, P_NG, P_H, P_I, P_J,
                            P_K, P_L, P_M, P_N, P_O, P_P, P_K,  P_R, P_S, P_T,
                            P_U, P_V, P_U, P_S, P_Y, P_S, P_AE, P_OE};

namespace SoLoud
{
VizsnResonator::VizsnResonator()
    : a(0)
    , b(0)
    , c(0)
    , p1(0)
...
unsigned int VizsnInstance::getAudio(float*       aBuffer,
...
float VizsnInstance::vcsrc(int aPitch, int aVoicetype)
{
    ...
    switch (aVoicetype)
    {
        ...
        case 1:
            return (float)(sin(mA * 0.0002) * cos(mA * 0.0003) * 0.2f +
                           ((rand() % 200 - 100) / 300.0f)); // ilmava
        ...
        case 8:
            return (float)(rand() % (1 + ((mA & 65535) >> 8))) /
                   256; // -- hiukka ihmisempi tsaatana
