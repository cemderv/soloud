...
namespace SoLoud {
  ...
  unsigned int MonotoneInstance::getAudio(float* aBuffer, unsigned int aSamplesToRead, unsigned int /*aBufferSize*/) {
    ...
    unsigned int i;
    for (i = 0; i < 12; i++) {
      mOutput[i].mEnabled = i < (unsigned int)mParent->mHardwareChannels && i < (unsigned int)mParent->mSong.
                            mTotalTracks;
    }
        ...
        for (j = 0; j < mParent->mSong.mTotalTracks; j++) {
          if (mChannel[j].mActive) {
            if (mChannel[j].mVibrato) {
              mChannel[j].mFreq[0] = mParent->mNotesHz[
                mChannel[j].mLastNote * 8 + (mParent->mVibTable[mChannel[j].mVibratoIndex] * mChannel[j].mVibratoDepth)
                / 64];
          ...
          if (mChannel[mNextChannel].mActive) {
            if (mChannel[mNextChannel].mArp) {
              mOutput[gotit].mSamplePosInc =
                  1.0f / (mSamplerate / mChannel[mNextChannel].mFreq[mChannel[mNextChannel].mArpCounter]);
