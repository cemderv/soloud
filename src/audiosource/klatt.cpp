...
void klatt::frame_init()
{
    ...
    */
    ...
    mParallelFormant1.initResonator(mFrame.mFormant1Freq,
                                    mFrame.mFormant1ParallelBandwidth,
                                    mSampleRate);
    mParallelFormant1.setGain(amp_parF1);

    mParallelResoNasalPole.initResonator(mFrame.mNasalPoleFreq,
                                         mFrame.mNasalPoleBandwidth,
                                         mSampleRate);
    mParallelResoNasalPole.setGain(amp_parFN);

    mParallelFormant2.initResonator(mFrame.mFormant2Freq,
                                    mFrame.mFormant2ParallelBandwidth,
                                    mSampleRate);
    mParallelFormant2.setGain(amp_parF2);

    mParallelFormant3.initResonator(mFrame.mFormant3Freq,
                                    mFrame.mFormant3ParallelBandwidth,
                                    mSampleRate);
    mParallelFormant3.setGain(amp_parF3);

    mParallelFormant4.initResonator(mFrame.mFormant4Freq,
                                    mFrame.mFormant4ParallelBandwidth,
                                    mSampleRate);
    mParallelFormant4.setGain(amp_parF4);

    mParallelFormant5.initResonator(mFrame.mFormant5Freq,
                                    mFrame.mFormant5ParallelBandwidth,
                                    mSampleRate);
    mParallelFormant5.setGain(amp_parF5);

    mParallelFormant6.initResonator(mFrame.mFormant6Freq,
                                    mFrame.mFormant6ParallelBandwidth,
                                    mSampleRate);
...
int klatt::synth(int /* aSampleCount */, short* aSamplePointer)
{
    ...
    {
        Element* ne =
            (mElementIndex < mElementCount) ? &gElement[mElement[mElementIndex]] : &gElement[0];
        ...
        for (t = 0; t < dur; t++, mTStress++)
        {
            ...
            {
                tp[j] = interpolate(&start[j],
                                    &end[j],
                                    (float)currentElement->mInterpolator[j].mSteady,
                                    t,
                                    dur);
            }
