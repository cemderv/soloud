  ...
  void Soloud::mixBus_internal(float*       aBuffer,
    ...
    for (i = 0; i < mActiveVoiceCount; i++) {
      ...
      if (voice &&
        ...
        while (step_fixed != 0 && outofs < aSamplesToRead) {
            ...
            if (!voice->hasEnded() || voice->mFlags & AudioSourceInstance::LOOPING) {
              ...
              if (readcount < SAMPLE_GRANULARITY) {
                if (voice->mFlags & AudioSourceInstance::LOOPING) {
                  while (readcount < SAMPLE_GRANULARITY && voice->seek(voice->mLoopPoint, mScratch.mData, mScratchSize)
                         == SO_NO_ERROR) {
                    voice->mLoopCount++;
          ...
          if (writesamples) {
            for (j = 0; j < voice->mChannels; j++) {
              switch (aResampler) {
                case RESAMPLER_POINT: resample_point(voice->mResampleData[0] + SAMPLE_GRANULARITY * j,
                                                     ...
                                                     step_fixed);
                  break;
                case RESAMPLER_CATMULLROM: resample_catmullrom(voice->mResampleData[0] + SAMPLE_GRANULARITY * j,
                                                               voice->mResampleData[1] + SAMPLE_GRANULARITY * j,
                                                               aScratch + aBufferSize * j + outofs,
                                                               voice->mSrcOffset,
                                                               writesamples,
        ...
        if (!(voice->mFlags & (AudioSourceInstance::LOOPING | AudioSourceInstance::DISABLE_AUTOSTOP)) && voice->
            hasEnded()) {
              ...
              if (readcount < SAMPLE_GRANULARITY) {
                if (voice->mFlags & AudioSourceInstance::LOOPING) {
                  while (readcount < SAMPLE_GRANULARITY && voice->seek(voice->mLoopPoint, mScratch.mData, mScratchSize)
                         == SO_NO_ERROR) {
                    voice->mLoopCount++;
                    readcount += voice->getAudio(voice->mResampleData[0] + readcount,
                                                 SAMPLE_GRANULARITY - readcount,
                                                 SAMPLE_GRANULARITY);
                  }
                }
